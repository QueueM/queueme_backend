...
class EmployeeWorkingHoursModel(models.Model):
    ...
    employee = models.ForeignKey(
        'employeeApp.EmployeeDetailsModel',
    ...
    def __str__(self):
        if not self.is_closed:
            return f"{self.employee.name} - {self.day}: {self.start_time} to {self.end_time}"
        return f"{self.employee.name} - {self.day}: Not working"
...
class EmployeeDetailsModel(models.Model):
    ...
    # New field to store AI performance metrics and recommendations
    ai_performance_data = models.JSONField(
        blank=True, 
        null=True, 
        help_text="AI performance metrics and recommendations"
    )
