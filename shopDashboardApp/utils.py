...
def compute_and_broadcast_dashboard(sender, instance, created, deleted, **kwargs):
    ...
    def build_metrics(qs, shop=None):
        ...
        avg_dur = qs.filter(status='booked').aggregate(avg=Avg('service__duration'))['avg'] or timedelta(0)
        est_wait = int(counts['waiting'] * (avg_dur.total_seconds() / 60))
        ...
        top_qs = (qs.values('service__name')
...
booking_changed.connect(compute_and_broadcast_dashboard)